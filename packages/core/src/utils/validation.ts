...
import { AuthenticationError, FatalError } from '../errors.js';
...
/**
 * Validate and sanitize pipeline options received from an untrusted source
 * (IPC, socket, etc.). TypeScript types are stripped at runtime so validation
 * is required.
 */
export function validatePipelineOptions(options: unknown): ValidatedPipelineOptions {
  ...
  const language =
    typeof opts.language === 'string' && VALID_LANGUAGE_RE.test(opts.language)
      ? opts.language
      : 'en-US';
...
/** Sanitize error messages before sending to an untrusted client — strip file paths and sensitive data */
export function sanitizeErrorMessage(err: unknown): string {
  if (!(err instanceof Error)) return 'An unexpected error occurred';

  // For authentication errors, pass through the user-friendly message
  if (err instanceof AuthenticationError || err instanceof FatalError) {
    return err.message;
  }
